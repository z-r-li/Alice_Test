...
class TestReadonlyOpen:
    """``readonly=True`` 真只读开库（PR #90 复审，Codex）：SQLite ``mode=ro`` URI，
    零 DDL / 零写——写路径被 OS 层拒绝；旧库缺迁移列 → 开库门禁抛明确指引
    （fail-closed），而非静默迁移改 schema 或深处炸 no such column。"""

    def _seed(self, tmp_path):
        db = tmp_path / "ro.db"
        with SQLiteStore(str(db)) as s:
            s.save_decision(_entry(gap=5.0))
            s.record_outcome(OutcomeEntry(decision_id="601985-2026-06-29",
                                          is_final=1, hit=1, excess_return=0.03))
        return db

    def test_reads_work_and_db_bytes_untouched(self, tmp_path):
        db = self._seed(tmp_path)
        before = db.read_bytes()
        with SQLiteStore(str(db), readonly=True) as s:
            assert s.get_decisions()[0]["decision_id"] == "601985-2026-06-29"
            assert s.hit_rate() == pytest.approx(1.0)
            assert s.information_coefficient("gap") is None   # 单点样本 → None，不编造
            assert s.get_open_decisions() == []
        assert db.read_bytes() == before                      # 零写：库文件逐字节未动
    ...
    def test_writes_rejected(self, tmp_path):
        ...
        with SQLiteStore(str(db), readonly=True) as s:
            with pytest.raises(sqlite3.OperationalError, match="readonly"):
                s.save_decision(_entry(decision_id="new", asof_date="2026-07-01"))
            with pytest.raises(sqlite3.OperationalError, match="readonly"):
                s.record_outcome(OutcomeEntry(decision_id="601985-2026-06-29", is_final=1))
    ...
    def test_missing_file_not_created(self, tmp_path):
        ...
        assert not db.exists()                                # mode=ro 绝不静默建库
    ...
    def test_legacy_db_fails_closed_with_migration_guidance(self, tmp_path):
        ...
        # decision_outcome 无 created_at——只读打开必须点名缺列并指引先迁移。
        import sqlite3
        db = str(tmp_path / "legacy_ro.db")
        conn = sqlite3.connect(db)
        conn.executescript(
            f"""
            CREATE TABLE decision_log (
        ...
        with pytest.raises(RuntimeError) as ei:
            SQLiteStore(db, readonly=True)
        msg = str(ei.value)
        assert "signal_semantics" in msg and "created_at" in msg   # 点名缺列
        assert "可写连接" in msg                                    # 指引先迁移
        # 指引闭环：按提示用可写连接打开一次（自动迁移）后，只读开库即通
        SQLiteStore(db).close()
        with SQLiteStore(db, readonly=True) as s:
            assert s.get_decisions() == []
